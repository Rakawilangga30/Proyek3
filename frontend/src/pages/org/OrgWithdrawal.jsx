...
export default function OrgWithdrawal() {
    ...
    const fetchData = async () => {
        try {
            const [balanceRes, historyRes] = await Promise.all([
                ...
                api.get('/organization/withdrawals')
            ]);
    ...
    const handleSubmit = async (e) => {
        ...
        const confirmMsg = `Konfirmasi Penarikan Organisasi:\n\n` +
        ...
        try {
            const response = await api.post('/organization/withdraw', {
                amount: amount,
    ...
    return (
        <div>
            ...
            <div style={{ marginBottom: "32px" }}>
                <Link to="/dashboard/org" style={{ color: "#3b82f6", textDecoration: "none", fontSize: "0.9rem", display: "inline-block", marginBottom: "12px" }}>
                    ← Kembali ke Dashboard
                </Link>
                <h1 style={{ margin: "0 0 8px 0", color: "#1e293b", fontSize: "1.75rem" }}>💸 Tarik Dana Organisasi</h1>
                <p style={{ margin: 0, color: "#64748b" }}>Tarik pendapatan organisasi ke rekening bank atau e-wallet</p>
            </div>
